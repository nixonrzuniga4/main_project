...
let matrix_vector_mul m v = 
  Array.map (fun row -> dot_product row v) m
...
let rnn_step params input dropout_rate =
  ...
  let h_new = vector_add (matrix_vector_mul params.wih input_with_dropout)
                         (matrix_vector_mul params.whh params.h) in
  let h_new = Array.map tanh h_new in  
  let h_new_with_dropout = dropout dropout_rate h_new in 
  let output = vector_add (
    matrix_vector_mul params.who h_new_with_dropout) params.bho in
...
let derivative_categorical_cross_entropy_loss predicted actual =
  let n = Array.length predicted in
  Array.init n (fun i -> 
    Array.init (Array.length predicted.(i)) (fun j -> 
      predicted.(i).(j) -. actual.(i).(j)
    )
  )

(** Derivative of tanh *)
let derivative_tanh x =
  1.0 -. (tanh x) ** 2.0
  ...
  if rows > 0 then
    ...
    for i = 0 to rows - 1 do
      for j = 0 to cols - 1 do
        parameter.(i).(j) <- parameter.(i).(j) -. learning_rate *. 
        gradient.(i).(j);
      done;
    done;;
...
let bptt rnn input_sequence target_sequence learning_rate =
  ...
  for t = num_timesteps - 1 downto 0 do
    let dy = derivative_categorical_cross_entropy_loss [|outputs.(t)|] 
    [|target_sequence.(t)|] in
    for i = 0 to output_size - 1 do
      for j = 0 to hidden_size - 1 do
        dL_dwho.(i).(j) <- dL_dwho.(i).(j) +. dy.(0).(i) *. 
        hidden_states.(t).(j);
      done;
    ...
    for t_step = t downto max 0 (t - truncate_backprop_steps) do
      let input = if t_step = 0 then create_vector input_size else 
        input_sequence.(t_step - 1) in
      for i = 0 to hidden_size - 1 do
        for j = 0 to input_size - 1 do
          dL_dwih.(i).(j) <- dL_dwih.(i).(j) +. !dL_dh.(i) *. input.(j);
        done;
        for j = 0 to hidden_size - 1 do
          dL_dwhh.(i).(j) <- dL_dwhh.(i).(j) +. !dL_dh.(i) *. 
          hidden_states.(t_step).(j);
        done;
