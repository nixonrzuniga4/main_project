...
(* Define variables outside the let () = block *)
let complete_tokenized_data = tokenize_nested_list 
(randomize_file "dataset/data/pickUpLines.txt")
...
let line_lengths = 
  List.flatten (
    List.map 
      (fun lines -> 
         List.map 
           (fun line -> List.length (tokenize_sentence line))
           lines)
...
(** Function to get the number of unique words in the vocabulary *)
let vocab_size vocab =
  Hashtbl.length vocab

let vocab_creation = (adding_treshold complete_tokenized_data 2)
let size = vocab_size vocab_creation
let (input_seqs, target_seqs) = create_sequences complete_tokenized_data 
vocab_creation
let (encoded_input_seqs, encoded_target_seqs) = prepare_data_for_rnn 
input_seqs target_seqs size
...
let prepare_data_for_epoch vocab =
  ...
  let (input_seqs, target_seqs) = create_sequences shuffled_data vocab_creation 
  in
  let (encoded_input_seqs, encoded_target_seqs) = 
  prepare_data_for_rnn input_seqs target_seqs size in
...
let train_rnn rnn epochs learning_rate batched_input_seqs batched_target_seqs =
  ...
  for epoch = 1 to epochs do
    ...
    let (batched_input_seqs, batched_target_seqs) = prepare_data_for_epoch 
    vocab_creation in
    ...
    List.iter2 (fun batch_input batch_target ->
      let batch_loss = ref 0.0 in
      let batch_size = List.length batch_input in
      batch_count := !batch_count + 1;

      List.iter2 (fun input_sequence target_sequence ->
        let predicted_outputs = List.map (fun input -> rnn_step rnn input 0.3) 
        input_sequence in
        let predicted_array = Array.of_list predicted_outputs in
        let target_array = Array.of_list target_sequence in
        let loss = categorical_cross_entropy_loss predicted_array target_array 
      in
        batch_loss := !batch_loss +. loss;
        bptt rnn (Array.of_list input_sequence) (Array.of_list target_sequence) 
        current_lr;
...
let find_index predicate array =
  let rec aux idx =
    if idx >= Array.length array then
      -1 
    else if predicate array.(idx) then
      idx
    else
      aux (idx + 1)
  in
...
let sample_from_distribution distribution =
  ...
  let _ = Array.fold_left (fun acc prob -> 
  ...
  let rec find_index cumul idx =
    if idx >= Array.length cumul then
      Array.length cumul - 1 
    else if cumul.(idx) >= rand then
      idx
    else
      find_index cumul (idx + 1)
  in
...
let generate_text rnn seed_text max_length vocab vocab_inv temp  =
  ...
  let seed_indices = List.map (fun token ->
    match Hashtbl.find_opt vocab token with
    | Some idx -> idx
    | None -> Hashtbl.find vocab "<UNK>"
  ) seed_tokens in
  let seed_vector = List.map (fun idx -> one_hot_encode idx size) 
  seed_indices in
  let generated_text = ref seed_text in

  (* Initialize current input with the last token of the seed text *)
  ...
  for _ = 1 to max_length do
    ...
    generated_text := !generated_text ^ 
                      (if String.contains ",.?!;:" next_token.[0] 
                        then next_token
                       else " " ^ next_token);
...
let () =
  ...
  Hashtbl.iter
  (fun key value -> Hashtbl.add vocab_inv value key) vocab_creation;
