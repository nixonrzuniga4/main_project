...
let tokenize_nested_list nested_list =
    List.map (fun inner_list ->
        List.flatten (List.map tokenize_sentence inner_list)
        ) nested_list
...
let count_word_frequencies tokenized_lines =
    ...
    List.iter (List.iter (fun token ->
        Hashtbl.replace freqs token (1 + 
        (try Hashtbl.find freqs token with Not_found -> 0))
    )) tokenized_lines;
    freqs
...
let adding_treshold tokenized_lines threshold =
    ...
    Hashtbl.iter (fun token freq ->
        if freq > threshold then
        (Hashtbl.add vocab token !counter; incr counter)
        else if not (Hashtbl.mem vocab "<UNK>") then
        (Hashtbl.add vocab "<UNK>" !counter; incr counter)
    ) freqs;
    vocab
...
let prepare_data_for_rnn input_seqs target_seqs vocab_size =
    let encode_seq seq = List.map (fun token ->
         one_hot_encode token vocab_size) seq in
...
let pad_sequences sequences max_length vocab_size =
  List.map (fun seq ->
    let padded = Array.to_list
    (Array.make max_length (padding_token vocab_size)) 
in
    let len = min (List.length seq) max_length in
    List.rev (List.fold_left (fun acc x -> x :: acc) (List.tl (List.rev padded)) 
    (List.rev (take len seq)))) sequences
...
let create_padding sequences batch_size max_length vocab_size =
  let padded_sequences = pad_sequences sequences max_length 
  vocab_size in
...
let percentile p lst =
    if p < 0. || p > 100. 
        then invalid_arg "percentile: p must be between 0 and 100";
    let sorted = sort_list lst in
    let index = int_of_float (p /. 100. *. float_of_int 
    (List.length sorted)) in
