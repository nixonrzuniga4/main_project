...
(* Identfies simple text within text layout, for this example if we type in "Hello!"
   the chatbot will say Hello! in return.*)
...
(* Main application of running the window. Objects that the window should start out
with the inlcusion of initial features. *)
let () =
  ...
  let text_input_box = W.text_input ~size:25 ~prompt:"Type and chat away ;) " () in
  let text_layout = L.resident ~x:550 ~y:900 ~w:500 ~h:100 text_input_box in
  let action b = print_endline (Format.asprintf "Button: %b" b) in
  let button = W.button ~action "Insert chat" in
